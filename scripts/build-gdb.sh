...
CFLAGS="-DHAVE_PSIGNAL -m32 ${PROFILING_OPTS} -s ALLOW_MEMORY_GROWTH=1 -s DISABLE_EXCEPTION_CATCHING=0 -fexceptions -s USE_PTHREADS=0" 
CPPFLAGS="-DHAVE_PSIGNAL -m32 ${PROFILING_OPTS} -s ALLOW_MEMORY_GROWTH=1 -s DISABLE_EXCEPTION_CATCHING=0 -fexceptions -s USE_PTHREADS=0" 
LDFLAGS="-Wl,--no-check-features -s DISABLE_EXCEPTION_CATCHING=0 -s USE_PTHREADS=0"
...
patch -N -p1 < ../../patches/gdb-emscripten-no-pthread.patch
